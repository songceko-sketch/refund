...
async function sendEmail(to, subject, html) {
  ...
  const replyTo = process.env.EMAIL_REPLY_TO || (process.env.ADMIN_EMAIL || 'info@refglob.com');

    if (!apiKey) throw new Error('RESEND_API_KEY is not set');

    // Create a plain-text fallback to improve deliverability
    const htmlToText = (h) => h.replace(/<style[\s\S]*?>[\s\S]*?<\/style>/gi, '')
